...
eiil = EIIL(
    logits=get_model_outputs(model, valset, device, verbose=True),
...
val_group_partition = eiil.infer_groups()
...
early_stopping_val_evaluator = Evaluator(
    testset=valset,
    group_partition=valset.group_partition,
    group_weights=valset.group_weights,
    batch_size=args.batch_size,
...
early_stopping_val_evaluator.evaluate()
results["val_early_stopping_worst_group_accuracy"] = early_stopping_val_evaluator.worst_group_accuracy[1]
results["val_early_stopping_average_accuracy"] = early_stopping_val_evaluator.average_accuracy 
